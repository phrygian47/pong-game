...
class RaylibPhysicsService(PhysicsService):
    ...
    def is_above(self, subject, agent):
        subject_rectangle = self._get_rectangle(subject)
        agent_rectangle = self._get_rectangle(agent)
        subject_rectangle_top = (subject_rectangle.y + subject_rectangle.height, subject_rectangle.width)
        agent_rectangle_top = (agent_rectangle.y - agent_rectangle.height, agent_rectangle.width)
        return pyray.check_collision_recs(subject_rectangle_top, agent_rectangle_top)

    def is_below(self, subject, agent):
        subject_rectangle = self._get_rectangle(subject)
        agent_rectangle = self._get_rectangle(agent)
        subject_rectangle_bottom = (subject_rectangle.y - subject_rectangle.height, subject_rectangle.width)
        agent_rectangle_bottom = (agent_rectangle.y + agent_rectangle.height, agent_rectangle.width)
        return pyray.check_collision_recs(subject_rectangle_bottom, agent_rectangle_bottom)
